...
sim_matching <- function(dag, n_sim, n_repeats, max_t, n_cores, seed=runif(1),
  ...
  out_final <- foreach::foreach(i=seq_len(n_repeats), .packages=pckgs,
    ...
    ## either perform matching on all covariates (time-dependent and fixed),
    ## or only on fixed and add info on time-dependent variables afterwards
    if (full_matching) {
...
get_data_for_match_td <- function(sim, treat="vacc", event="influenza",
  ...
  data <- list2start_stop(dlist, n=nrow(d_baseline), max_t=max_t,
                          durations=durations)
...
d_match <- match_td.fit(id=".id", time=".time",
...
data <- get_data_for_match_td(sim, treat="vacc", event="influenza",
                              baseline_vars="mac")

d_match <- match_td(vacc ~ mac, data=data, id=".id", event="influenza",
                    inclusion="inclusion", verbose=TRUE)
...
Hier sieht man bereits das es nicht sonderlich gut funktioniert. Das liegt aber nicht daran, dass nicht für die zeitabhängige Variable gematcht wurde, sondern daran das die Annahmen für das Cox Modell hier tatsächlich nicht mehr gelten! Hier eine Auswertung bei der stattdessen IPTW verwendet wird:
...
Die Ergebnisse sind hier deutlich näher an den korrekten Werten.
...
d_match2 <- match_td(id=".id", time=".time", d_treat=d_treat,
                    d_event=d_event, d_covars=d_covars,
                    keep_all_columns=TRUE, d_inclusion=d_inclusion,
                    replace_at_t=FALSE, verbose=TRUE)
...
Hier zeigt sich fast genau der gleiche Bias, da eben auch hier die Proportional Hazards Annahme verletzt ist. Aber da hier durch das vollständige Matching kein Confounding mehr vorliegt könnten wir hier auch einfache Kaplan-Meier Schätzer verwenden:
...
Das Problem lässt sich entweder durch vollständiges Matching lösen, oder durch die Verwendung einer anderen Adjustierungsmethode, welche nicht auf der Proportional-Hazards Annahme beruhen.

## Monte-Carlo Simulation ohne Matching auf meds
