...
estimate_target <- function(d_match, form_cox, t, method="g_cox") {
  ...
  if (method=="g_cox") {
    ...
    formula <- stats::as.formula(form_cox)
    mod <- aalen(Surv(event_time, status) ~ interact, data=d_match)
...
Hier sieht man bereits das es nicht sonderlich gut funktioniert. Das liegt aber nicht daran, dass nicht für die zeitabhängige Variable gematcht wurde, sondern daran das die Annahmen für das Cox Modell hier tatsächlich nicht mehr gelten! Im Anschluss zunächst die Auswertung der gleichen Daten, aber mit perfektem Matching auch auf "meds".
...
Hier zeigt sich fast genau der gleiche Bias. Aber da hier durch das vollständige Matching kein Confounding mehr vorliegt könnten wir hier auch einfache Kaplan-Meier Schätzer verwenden:
