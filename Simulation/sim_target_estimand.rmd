...
get_data_01 <- function(sim, d_influ) {
  ...
  d1[, event_time := .next_event_time - .treat_time]
  d1[, status := !is.na(event_time)]
  d1[is.na(event_time), event_time := 365 - .treat_time]
...
estimate_target <- function(d_match, form_cox, t) {
...
prob_influ_2 <- function(data, base_p, rr_mac) {
  ...
  fifelse(!data$vacc_event, as.numeric(data$influenza_event),
          fifelse(data$vacc_event &
                  (data$vacc_time - data$influenza_time) > 1, 1, p, na=p))
}
...
ggplot(sim1, aes(x=rr_mac)) +
  geom_histogram() +
  geom_vline(xintercept=median(true_target$rr_mac)) +
  geom_vline(xintercept=mean(sim1$rr_mac), linetype="dashed") +
  geom_vline(xintercept=median(sim1$rr_mac), linetype="dotted")
```
...
test <- sim_discrete_time(modified_dag, n_sim=100000, max_t=365)
...
```{r}
dat1 <- km2$adj
dat1$surv_est <- surv$adj$surv
dat1$diff <- (1 - dat1$surv) - (1 - dat1$surv_est)

ggplot(dat1, aes(x=time, y=diff, color=group)) +
  geom_line() +
  theme_bw()
