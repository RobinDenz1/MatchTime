...
```{r}
## compare standard cox with time-dependent matching
sim_comp2 <- function(dag, n_sim, n_repeats, max_t, n_cores, seed=runif(1),
                      td_variables=NULL, ...) {
...
sim_true_estimand <- function(n_sim, n_repeats, max_t, n_cores,
                              seed=runif(1)) {
  ...
  out_final <- foreach::foreach(i=seq_len(n_repeats), .packages=pckgs,
    ...
    dag <- empty_dag() +
      ...
      node_td("meds", type="time_to_event", prob_fun=prob_meds,
              event_duration=Inf, parents="vacc_event", base_p=0.001,
              rr_vacc=4) +
      node_td("vacc", type="time_to_event", prob_fun=1,
              event_duration=Inf) +
      node_td("influenza", type="time_to_event", prob_fun=prob_influ,
              event_duration=Inf, parents=c("mac", "vacc_event",
                                            "meds_event"),
              base_p=0.001, rr_mac=1.5, rr_vacc_mac=0.8,
              rr_vacc_nonmac=0.5, rr_meds=4)


    #set.seed(seed_i)
    sim1 <- sim_discrete_time(dag=dag, n_sim=n_sim, max_t=max_t)
    ...
    dag <- empty_dag() +
      ...
      node_td("meds", type="time_to_event", prob_fun=prob_meds,
          event_duration=Inf, parents="vacc_event", base_p=0.001,
          rr_vacc=4) +
      node_td("vacc", type="time_to_event", prob_fun=0,
          event_duration=Inf) +
      node_td("influenza", type="time_to_event", prob_fun=prob_influ,
              event_duration=Inf, parents=c("mac", "vacc_event",
                                            "meds_event"),
    ...
    #set.seed(seed_i)
    sim0 <- sim_discrete_time(dag=dag, n_sim=n_sim, max_t=max_t)

    d0 <- sim0$data
    d1 <- sim1$data
    d_target <- rbind(d0, d1)

    d_target[is.na(influenza_time), influenza_time := max_t]
...
test <- sim_true_estimand(n_sim=10000, n_repeats=500, max_t=365,
                          n_cores=5, seed=443)

true_rr_mac <- mean(test$rr_mac)
true_rr_nonmac <- mean(test$rr_nonmac)
true_interact <- mean(test$interact)


sim1 <- sim_comp2(dag=dag,
                  n_sim=10000,
                  max_t=365,
                  n_repeats=500,
                  n_cores=5,
...
sim1[, interact := (1 - rr_mac) / (1 - rr_nonmac)]
