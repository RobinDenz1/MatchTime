...
More specifically, we would say that the value of `bmi` for individual 1 was 32 from 0 up to and including t = 10. An infinitesimally amount of time later it changed to 34 and stayed on that value until t = 50. This type of coding is known as **right-open** or **left-closed** intervals (because the `start` value is considered to be in the interval, while the `stop` value is not). This is usually denoted as `[start, stop)` and is the usual way these intervals are coded for time-to-event purposes. Throughout this package we will rely exclusively on this type of intervals.
...
Most users will only need the `merge_td()` function to create a single start-stop dataset from different datasets containing time-dependent information. This function has the advantage that it does not blow up the dataset to the long-format, making it feasible to create start-stop data for very large datasets. The other, more advanced, functions are most useful when dealing with messy real-world data.
